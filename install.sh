#!/usr/bin/env bash
# install.sh — install the mundus-animarum plugin into an objectiveai dir
# (default $HOME/.objectiveai), the way the host's `plugins install` works.
#
...
REPO="ObjectiveAI/mundus-animarum"
...
# Re-use the existing artifacts only if BOTH are present (the cli_zip + the
# manifest); otherwise fetch both from the release.
if [ -f "$CLI_DIR/$CLI_ZIP_NAME" ] && [ -f "$PLUGIN_DIR/objectiveai.json" ]; then
