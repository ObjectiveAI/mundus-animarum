...
# objectiveai host binaries and the unpacked mundus-animarum plugin. test.sh
# does NOT build — run build.sh first (or build-and-test.sh). It resets per-run
# state, applies the api config the run needs, then runs cargo-nextest.
#
...
# 3. Global api config the run needs. mcp-timeout-ms keeps the MCP-heavy resume
